...
function Mine_A_Thon() {
  ...
  let { contact, phoneno, name, img, info, prize, teamLen, rules, ps, submission } = Comp.find((obj) => obj.id === "Mine_A_Thon");
  ...
  useEffect(() => {
    ...
    axios
      .get(`/auth/team/getMyTeam/Mine_A_Thon/${curUser._id}`)
      .then((res) => {
        if (res.data.success) {
          if (res.data.team) {
            ...
            <div id="off_events">
              <div className="off_events">
                ...
                <div className="off_events_right" style={{justifyContent:"start"}} >
                  ...
                  {team && (
                    <>
                      <TeamDash event={"Mine_A_Thon"} team={team} teamSize={teamLen} />
                    </>
                  )}
                </div>
              </div>
