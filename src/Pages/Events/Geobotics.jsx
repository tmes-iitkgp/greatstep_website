...
function Geobotics() {
  ...
  let {
    ...
    ps,
    submission,
  } = Comp.find((obj) => obj.id === "Geobotics");
  ...
  useEffect(() => {
    ...
    axios
      .get(`/auth/team/getMyTeam/Geobotics/${curUser._id}`)
      .then((res) => {
        if (res.data.success) {
          if (res.data.team) {
                        ...
                        {teamDash ? (
                          <>
                            <h2 className="addmember_alert">
                              You have successfully registered, You can now add
                              team members
                            </h2>
                            {/* <a
                              target="blank"
                              href={`https://docs.google.com/forms/d/e/1FAIpQLSd_xWjFkhUnAr1eilXzeD_qqfuCKZmlHiG8R5SS_IL19vt9nQ/viewform?usp=pp_url&entry.92426558=${
                                curUser.firstName + " " + curUser.lastName
                              }&entry.676221097=${
                                curUser.email
                              }&entry.1583837136=${getCollege(
                                curUser.college
                              )}&entry.1746928591=${curUser.mobile}`}
                            >
                              <button className="custom_btn2">
                                submission
                              </button>
                            </a> */}
                          </>
