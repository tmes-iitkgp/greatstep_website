...
function Mineac() {
  ...
  let { contact, phoneno, name, img, info, prize, teamLen, rules, ps, submission } = Comp.find((obj) => obj.id === "Mineac");
  ...
  useEffect(() => {
    ...
    axios
      .get(`/auth/team/getMyTeam/Mineac/${curUser._id}`)
      .then((res) => {
        if (res.data.success) {
          if (res.data.team) {
            ...
            <div id="off_events">
              <div className="off_events">
                ...
                <div className="off_events_right">
                  ...
                  {team && (
                    <>
                      <TeamDash event={"Mineac"} team={team} teamSize={teamLen} />
                    </>
                  )}
                </div>
              </div>
